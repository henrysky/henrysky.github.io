...
ocrid = "0000-0001-6855-442X"
# lower case for possible names
possible_names = ["jo bovy", "j. bovy"]
standardized_names = "Jo Bovy"
...
for paper in result['response']['docs']:
    ...
    if extra_data:
        ...
        if np.sum(idx) == 1:
            buttons_code += """<br class="d-none d-lg-block"><a target="_blank" href="{url}" class="btn btn-sm btn-info mt-1">Code</a>""".format(url=df["url"][idx].to_string()[1:].lstrip())
    buttons_code += """</div>"""
